...
# 1.2 特徵與目標
best_features = [
'風速', '氣壓', 'wave_dir_sin', 'wave_dir_cos', '降雨_YJ', '波能_log1p', '功率_log1p', '尖峰週期_BC'
]
...
# 2. 「先」全資料標準化 → 再切分
# ==========================
scaler = RobustScaler().fit(X)  # ★ 改動點：對「全部 X」fit
...
def evaluate_and_plot(model, Xmat, y_true, name):
    ...
    step = 10000
    x_min = np.floor(mn / step) * step
    x_max = np.ceil(mx / step) * step
