...
# 2. 先標準化全資料 → 再固定切分
# ==========================
# ★ 重點：先對「整份候選特徵矩陣」做 StandardScaler
...
for k in range(min_features, max_features + 1):
    for combo in itertools.combinations(candidate, k):
        ...
        if (r2_train > r2_min) and (r2_test > r2_min):
            results.append({
                'features': combo,
                'r2_train': r2_train,
                'r2_test':  r2_test
...
# ==========================
# 4. 整理並印出前 10 名（Top 10 by Train R²，且 Test R² > 0.4）
# ==========================
...
if not results_df.empty:
    # 過濾 Test R² 門檻，先以 Train R² 由大到小排序；同分再看 Test R²
    top = (results_df.loc[results_df['r2_test'] > test_thr]
                       .sort_values(['r2_train', 'r2_test'], ascending=[False, False])
                       .head(top_k)
                       .copy())

    if top.empty:
        print(f"⚠️ 沒有符合 Test R² > {test_thr} 的結果。")
    else:
        # 美化 features 顯示
        top['features'] = top['features'].apply(lambda t: '(' + ', '.join(map(str, t)) + ')')
        with pd.option_context('display.max_colwidth', 1000):
            print(f"✅ Top {len(top)} by Train R²（Test R² > {test_thr}）：")
            print(top.reset_index(drop=True).to_string(index=False))
else:
    print("⚠️ 尚無可用結果（results 為空）。")
